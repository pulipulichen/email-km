...
function wp_pagenavi( $args = array() ) {
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				...
				$options['pages_text'] );
				$out .= "<span class='item pages'>$pages_text</span>";
			}
			...
			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				if ( !empty( $options['dotleft_text'] ) )
					$out .= "<span class='disabled item extend'>{$options['dotleft_text']}</span>";
			}
			...
			if ( $larger_page_start )
				$out .= "<span class='item extend'>{$options['dotleft_text']}</span>";
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					$out .= "<span class='active item current'>$current_page_text</span>";
					$timeline = 'larger';
				} else {
					$out .= $instance->get_single( $i, $options['page_text'], array(
						'class' => "item page $timeline",
					) );
				}
			}
			...
			if ( $larger_page_out ) {
				$out .= "<span class='item extend'>{$options['dotright_text']}</span>";
			}
			$out .= $larger_page_out;
	...
	$out = $before . "<div class='wp-pagenavi-wrapper'><div class='wp-pagenavi ui pagination menu'>\n$out\n</div></div>" . $after;
