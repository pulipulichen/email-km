...
if (function_exists('wp_authenticate') || function_exists('wp_set_auth_cookie') || !function_exists('ldap_connect')) {
/**
 * SAFE MODE
 */
 ...
 */
    ...
    */
    ...
    function wpDirAuth_auth($username, $password)
    {
		...
		//throw new Exception("錯誤，就是錯誤");
        if ( ($preBindUser && $preBindPassword) && ( ! $isPreBound ) ) {
			throw new Exception("LDAP錯誤，請聯絡管理者");
		...
		elseif ( ( isset($isInDirectory) === TRUE && $isInDirectory === true ) && ( ! $isBound ) ) {
			//echo "[a]";
			//throw new Exception("a錯誤，就是錯誤");
				return new WP_Error ('could_not_bind_as_user', $errorTitle
					   . __(' Incorrect password.'));
		}
        elseif ( ! $isBound && ! $isPreBound ) {
			//echo "[b]";
			//throw new Exception("b錯誤，就是錯誤");
    ...
    function wpDirAuth_optionsPanel()
    {
		...
		$curUserIsDirUser = get_user_meta($userdata->ID, 'wpDirAuthFlag');
...
________EOS;
    ...
    function wp_authenticate($username, $password)
    {
        ...
        */
		...
		$loginUserIsDirUser = ($login) ? get_user_meta($login->ID, 'wpDirAuthFlag') : 0;
